...
import decord
import numpy as np
from src.modeling.video_llama.processors import transforms_video
from src.modeling.video_llama.processors.base_processor import BaseProcessor
from src.modeling.video_llama.processors.randaugment import VideoRandomAugment
from src.modeling.video_llama.processors import functional_video as F
...
def load_video(video_path, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", return_msg = False):
    ...
    if n_frms == 1:
        ...
        indices = np.arange(vlen)
        # Repeat indices until we reach n_frms
        repeat_factor = int(np.ceil(n_frms / vlen))
        indices = np.tile(indices, repeat_factor)[:n_frms]
